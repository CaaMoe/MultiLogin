...
public class CacheWhitelistTable {
     ...
     * @return 数据操作量
     */
    public int addCacheWhitelist(UUID onlineUUID, int yggdrasilId) throws SQLException {
        try (Connection connection = sqlManager.getPool().getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(
                     MessageFormat.format("INSERT INTO {0} ({1}, {2}) " +
                                     "SELECT ?, ? FROM DUAL WHERE NOT EXISTS(SELECT {1}, {2} FROM {0} WHERE " +
                                     "{1} = ? AND {2} = ?)",
                             tableName,
                             fieldOnlineUuid, fieldYggdrasilId
                     ))) {
     ...
     * @return 数据操作量
     */
    public int addCacheWhitelist(int yggdrasilId, String currentUsername) throws SQLException {
        try (Connection connection = sqlManager.getPool().getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(
                     MessageFormat.format("INSERT INTO {0} ({1}, {2}) " +
                                     "SELECT ?, ? FROM DUAL WHERE NOT EXISTS(SELECT {1}, {2} FROM {0} WHERE " +
     ...
     * @return 数据操作量
     */
    public int addCacheWhitelist(String currentUsername) throws SQLException {
        try (Connection connection = sqlManager.getPool().getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(
                     MessageFormat.format("INSERT INTO {0} ({1}) " +
                                     "SELECT ? FROM DUAL WHERE NOT EXISTS(SELECT {1} FROM {0} WHERE " +
                                     "{1} = ?)",
                             tableName, fieldCurrentUsername,
                             fieldYggdrasilId
                     ))) {
     ...
     * @return 数据操作量
     */
    public int addCacheWhitelist(UUID onlineUUID) throws SQLException {
        try (Connection connection = sqlManager.getPool().getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(
                     MessageFormat.format("INSERT INTO {0} ({1}) " +
                                     "SELECT ? FROM DUAL WHERE NOT EXISTS(SELECT {1} FROM {0} WHERE " +
