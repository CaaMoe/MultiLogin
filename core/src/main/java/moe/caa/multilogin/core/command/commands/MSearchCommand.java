    ...
    public LiteralArgumentBuilder<ISender> register(LiteralArgumentBuilder<ISender> literalArgumentBuilder) {
        return literalArgumentBuilder.then(handler.literal("login")
                        .then(handler.literal("byName")
                                .requires(sender -> sender.hasPermission(Permissions.COMMAND_MULTI_LOGIN_QUERY_LOGIN_BYNAME))
                        ...
                        .then(handler.literal("byInGameUUID")
                                .requires(sender -> sender.hasPermission(Permissions.COMMAND_MULTI_LOGIN_QUERY_LOGIN_BYINGAMEUUID))
                ...
                .then(handler.literal("inGameUUID")
                        .then(handler.literal("byName")
                                .requires(sender -> sender.hasPermission(Permissions.COMMAND_MULTI_LOGIN_QUERY_IN_GAME_UUID_BYNAME))
                                .then(handler.argument("username", StringArgumentType.string())
                                        .executes(this::executeInGameUUIDByName)
                                ))
                        .then(handler.literal("byProfile")
                                .requires(sender -> sender.hasPermission(Permissions.COMMAND_MULTI_LOGIN_QUERY_IN_GAME_UUID_BYPROFILE))
