...
public class SQLManager {
    ...
    public void init() throws Exception {
        YamlConfig config = ValueUtil.getOrThrow(MultiCore.getConfig().get("sql", YamlConfig.class), LanguageKeys.CONFIGURATION_KEY_ERROR.getMessage("sql"));
        ...
        String prefix = ValueUtil.getOrThrow(config.get("prefix", String.class), LanguageKeys.CONFIGURATION_VALUE_ERROR.getMessage("prefix"));
        if (prefix.length() != 0) {
        ...
        if (backend == SQLBackend.MYSQL) {
            ...
            url = String.format(url, MultiCore.getPlugin().getDataFolder().getAbsolutePath(), "/multilogin");
        ...
        MultiCore.log(LoggerLevel.INFO, LanguageKeys.DATABASE_CONNECTED.getMessage(backend.name()));
    }
