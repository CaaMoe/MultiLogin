...
public class SQLHandler {
    ...
    public static void init() throws Exception {
        ...
        String username = ValueUtil.getOrThrow(config.get("username", String.class), LanguageKeys.CONFIGURATION_KEY_ERROR.getMessage("username"));
        String password = ValueUtil.getOrThrow(config.get("password", String.class), LanguageKeys.CONFIGURATION_KEY_ERROR.getMessage("password"));
        String ip;
        int port;
        String database;
        String url;
        if (ValueUtil.getOrThrow(config.get("backend", SQLBackend.class), LanguageKeys.CONFIGURATION_KEY_ERROR.getMessage("backend")) == SQLBackend.MYSQL) {
            ip = ValueUtil.getOrThrow(config.get("ip", String.class), LanguageKeys.CONFIGURATION_KEY_ERROR.getMessage("ip"));
            database = ValueUtil.getOrThrow(config.get("database", String.class), LanguageKeys.CONFIGURATION_KEY_ERROR.getMessage("database"));
            port = ValueUtil.getOrThrow(config.get("port", Integer.class), LanguageKeys.CONFIGURATION_KEY_ERROR.getMessage("port"));
