...
/**
 * 此事件在 LoginEvent 事件中触发, 用于记录玩家的有效来源.
 * 当事件结束后没有设置一个有效的来源的话将拒绝当前玩家登录游戏.
 *
 * 需要注意的是, 如果事件中的玩家的游戏档案未曾在 MultiLogin 记录过(名称以及uuid全部匹配), 在当前事件结束后将拒绝玩家登录游戏.
 * 所以在实现 Service 时, 需要在 GameProfileRequestEvent 中手动把玩家在游戏内的档案调整为已登记入库的档案,
 ...
 * @see com.velocitypowered.api.event.player.GameProfileRequestEvent
 * @see com.velocitypowered.api.event.connection.LoginEvent
 */
class PlayerLoginSourceRequestEvent(
    ...
    fun setSource(source: LoginSource) = synchronized(this) {
        if (this.source != null) {
            throw LoginSourceRepeatSetException("duplicate source.")
        }

        if (MinimalProfile.fromGameProfile(player.gameProfile) != source.inGameProfile) {
