...
public record SignedCookieData<D extends CookieData>(D cookieData, String base64Signature, String base64SignContent) {
    ...
    public static SignedCookieData<?> readSignedCookieData(byte[] bytes) throws Throwable {
        ...
        String base64SignContent = parsedJsonObject.getAsJsonPrimitive("content").getAsString();
    ...
    public static <D extends CookieData> SignedCookieData<D> signData(D cookieData, PrivateKey key, String algorithm) throws Exception {
        JsonObject data = CookieData.serialize(cookieData);
